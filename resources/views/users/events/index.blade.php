...
@section('content')
    <div class="container-fluid align-items-center">
        <div class="m-5">
                ...
                <div id="carouselExampleIndicators" class="carousel slide" data-bs-ride="carousel">
                    <div class="carousel-indicators">
                        @for ($i = 0, $indicator = 1, $indicatorCount = 0; $i < count($all_events); $i++, $indicator++)
                            @if ($indicator == 3 || $indicator < 3 && $i + 1 == count($all_events))
                                @if ($i === 2)
                                    <button type="button" data-bs-target="#carouselExampleIndicators"
                                        data-bs-slide-to="{{ $indicatorCount }}" class="active" aria-current="true"
                                        aria-label="Slide {{ $indicatorCount + 1 }}"></button>
                                @else
                                    <button type="button" data-bs-target="#carouselExampleIndicators"
                                        data-bs-slide-to="{{ $indicatorCount }}"
                                        aria-label="Slide {{ $indicatorCount + 1 }}"></button>
                                @endif
                                @php($indicator = 1)
                                @php($indicatorCount++)
                        ...
                        @for ($i = 0; $i < count($all_events); $i++)
                            <div class="carousel-item {{ $i === 0 ? 'active' : '' }}">
                                <div class="row mt-5 mb-5">
                                    <!-- For the cards -->
                                    @for ($j = 0; $j < 3; $j++, $i++)
                                        @if ($i == count($all_events))
                                        @break
                                    @endif
                                    <div class="col-lg-4 col-md-6 gx-0 p-0 mx-auto event-info mb-5">
                                        <div class="card event-card mb-2">
                                            <img src="{{ asset('storage/images/' . $all_events[$i]->image) }}"
                                                class="card-img-top" alt="{{ $all_events[$i]->image }}">
            ...
            <button class="carousel-control-prev chevron chevron-left" type="button" data-bs-target="#carouselExampleIndicators"
                data-bs-slide="prev">
                <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                <span class="visually-hidden">Previous</span>
            </button>
            <button class="carousel-control-next chevron chevron-right" type="button" data-bs-target="#carouselExampleIndicators"
                data-bs-slide="next">
    ...
    @else
        <h1 class="text-center text-muted mt-5">Sorry<i class="fa-solid fa-face-sad-tear"></i>...No Event yet...
        </h1>
    @endif
@endsection
