...
<div class="container">
    <div class="form w-50 mx-auto p-5">
        ...
        <h2 class="display-5">{{ $event->theme }}</h2>
            <div class="line">
                <p class="join-event-members mb-0 px-4"><b>{{ $event->joinEvents->count() }}{{$event->joinEvents->count() == 1 ? 'member':'members'}}</b></p>
            </div>
                ...
                <div class="join-users my-2 mx-auto">
                    <div class="join-user">
                        @if($participant->user_id && $participant->user->avatar)
                            <img src="{{ asset('storage/avatars/'.$participant->user->avatar) }}" class="rounded-circle" id="avatar-sm">
                        @else
                    ...
                    @if($participant->user_id)
                        <button data-bs-toggle="modal" data-bs-target="#other-event-member{{ $participant->user->id }}" class="border-0 bg-white">
                                    ...
                                    <div class="modal-title text-center fs-2 my-3 fw-bold">OTHER MEMBER</div>
                                        <div class="avatar mx-auto">
                                            @if($participant->user->avatar)
                                                <img src="{{ asset('storage/avatars/'. $participant->user->avatar) }}" alt="" class="avatar-md">
                                            @else
                                                <i class="fa-solid fa-circle-user fa-2xl text-secondary text-center"></i>
                                            @endif
                                        </div>
                                        <div class="text-center">
                                            @if($participant->user_id)
                                                <img src="{{ asset('image/level/' . $participant->user->level->icon) }}" class="avatar-level mx-auto" alt="{{ $participant->user->level->name }}">
                                                <span class="h3 mx-auto" id="username">{{ $participant->user->user_name }}</span>
                                            @endif                                      
                                        </div>
                    ...
                    @else
                        <span class="category-item h5 my-0 ms-3 text-secondary">{{$participant->name}}</span>
                    @endif
